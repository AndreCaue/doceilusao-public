...
Built with **FastAPI**, DoceIlusão handles everything from product catalogs and cart management to subscription-gated video streaming via AWS S3 pre-signed URLs. Security and scalability are first-class citizens: role-based access control, strict scope validation, and full isolation between public and private cloud storage.
...
### Public Bucket (`DoceIlusão-public-prod`)
...
### Private Bucket (`DoceIlusão-private-prod`)
...
# Database
DATABASE_URL=postgresql://user:password@localhost:5432/DoceIlusão
...
S3_BUCKET_PUBLIC_DEV=DoceIlusão-public-dev
S3_BUCKET_PUBLIC_PROD=DoceIlusão-public-prod

# Private Buckets (Premium Content, Videos)
S3_BUCKET_PRIVATE_DEV=DoceIlusão-private-dev
S3_BUCKET_PRIVATE_PROD=DoceIlusão-private-prod
...
*DoceIlusão Backend — built with FastAPI. Contributions welcome.*
